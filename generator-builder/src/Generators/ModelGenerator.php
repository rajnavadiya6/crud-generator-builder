...
use Crud\GeneratorBuilder\Utils\TableFieldsGenerator;
...
class ModelGenerator extends BaseGenerator
{
    ...
    {
        $templateData = get_template('model.model', 'generator-builder');
    ...
    private function fillTemplate($templateData)
    {
        ...
        $templateData = str_replace(
            ...
            fill_template($this->commandData->dynamicVars,
                implode(PHP_EOL.infy_nl_tab(1, 1), $this->generateRelations())),
            $templateData
        );
    ...
    private function fillSoftDeletes($templateData)
    {
        if (! $this->commandData->getOption('softDelete')) {
            ...
            $deletedAtTimestamp = config('crud.generator_builder.timestamps.deleted_at', 'deleted_at');
            $templateData = str_replace(
                '$SOFT_DELETE_DATES$',
    ...
    private function fillDocs($templateData)
    {
        ...
        $docsTemplate = get_template('docs.model', 'generator-builder');
    ...
    {
        $dont_require_fields = config('crud.generator_builder.options.hidden_fields', [])
            + config('crud.generator_builder.options.excluded_fields', $this->excluded_fields);

...
namespace Crud\GeneratorBuilder\Common;
...
class GeneratorConfig
{
    ...
    public function loadNamespaces(CommandData &$commandData)
    {
        ...
        $this->nsRepository = config('crud.generator_builder.namespace.repository', 'App\Repositories').$prefix;
        $this->nsModel = config('crud.generator_builder.namespace.model', 'App\Models').$prefix;
        if (config('crud.generator_builder.ignore_model_prefix', false)) {
            $this->nsModel = config('crud.generator_builder.namespace.model', 'App\Models');
        }
        $this->nsSeeder = config('crud.generator_builder.namespace.seeder', 'Database\Seeders').$prefix;
        $this->nsFactory = config('crud.generator_builder.namespace.factory', 'Database\Factories').$prefix;
        $this->nsDataTables = config('crud.generator_builder.namespace.datatables', 'App\DataTables').$prefix;
        $this->nsModelExtend = config(
            'crud.generator_builder.model_extend_class',
            'Illuminate\Database\Eloquent\Model'
        );

        $this->nsApiController = config(
                'crud.generator_builder.namespace.api_controller',
                'App\Http\Controllers\API'
            ).$prefix;
        $this->nsApiRequest = config('crud.generator_builder.namespace.api_request', 'App\Http\Requests\API').$prefix;

        $this->nsRequest = config('crud.generator_builder.namespace.request', 'App\Http\Requests').$prefix;
        $this->nsRequestBase = config('crud.generator_builder.namespace.request', 'App\Http\Requests');
        $this->nsBaseController = config('crud.generator_builder.namespace.controller', 'App\Http\Controllers');
        $this->nsController = config('crud.generator_builder.namespace.controller', 'App\Http\Controllers').$prefix;

        $this->nsApiTests = config('crud.generator_builder.namespace.api_test', 'Tests\APIs');
        $this->nsRepositoryTests = config('crud.generator_builder.namespace.repository_test', 'Tests\Repositories');
        $this->nsTests = config('crud.generator_builder.namespace.tests', 'Tests');
    }
    ...
    public function loadPaths()
    {
        ...
        $this->pathRepository = config(
                'crud.generator_builder.path.repository',
                app_path('Repositories/')
            ).$prefix;

        $this->pathModel = config('crud.generator_builder.path.model', app_path('Models/')).$prefix;
        if (config('crud.generator_builder.ignore_model_prefix', false)) {
            $this->pathModel = config('crud.generator_builder.path.model', app_path('Models/'));
        }

        $this->pathDataTables = config('crud.generator_builder.path.datatables', app_path('DataTables/')).$prefix;

        $this->pathApiController = config(
                'crud.generator_builder.path.api_controller',
                app_path('Http/Controllers/API/')
            ).$prefix;

        $this->pathApiRequest = config(
                'crud.generator_builder.path.api_request',
                app_path('Http/Requests/API/')
            ).$prefix;

        $this->pathApiRoutes = config('crud.generator_builder.path.api_routes', base_path('routes/api.php'));

        $this->pathApiTests = config('crud.generator_builder.path.api_test', base_path('tests/APIs/'));

        $this->pathController = config(
                'crud.generator_builder.path.controller',
                app_path('Http/Controllers/')
            ).$prefix;

        $this->pathRequest = config('crud.generator_builder.path.request', app_path('Http/Requests/')).$prefix;

        $this->pathRoutes = config('crud.generator_builder.path.routes', base_path('routes/web.php'));
        $this->pathFactory = config('crud.generator_builder.path.factory', database_path('factories/'));

        $this->pathViews = config(
                'crud.generator_builder.path.views',
                resource_path('views/')
            ).$viewPrefix.$this->mSnakePlural.'/';

        $this->pathSeeder = config('crud.generator_builder.path.seeder', database_path('seeders/'));
        $this->pathDatabaseSeeder = config('crud.generator_builder.path.database_seeder',
            database_path('seeders/DatabaseSeeder.php'));
        $this->pathViewProvider = config(
            'crud.generator_builder.path.view_provider',
            app_path('Providers/ViewServiceProvider.php')
        );

        $this->modelJsPath = config(
            'crud.generator_builder.path.modelsJs',
    ...
    public function loadDynamicVariables(CommandData &$commandData)
    {
        ...
        $commandData->addDynamicVariable(
            '$API_PREFIX$',
            config('crud.generator_builder.api_prefix', 'api')
        );

        $commandData->addDynamicVariable(
            '$API_VERSION$',
            config('crud.generator_builder.api_version', 'v1')
        );
    ...
    public function prepareOptions(CommandData &$commandData)
    {
        ...
        if (empty($this->options['save'])) {
            $this->options['save'] = config('crud.generator_builder.options.save_schema_file', true);
        }

        if (empty($this->options['localized'])) {
            $this->options['localized'] = config('crud.generator_builder.options.localized', false);
        }
        ...
        $this->options['softDelete'] = config('crud.generator_builder.options.softDelete', false);
        $this->options['repositoryPattern'] = config('crud.generator_builder.options.repository_pattern', true);
    ...
    {
        $this->prefixes['route'] = explode('/', config('crud.generator_builder.prefixes.route', ''));
        $this->prefixes['path'] = explode('/', config('crud.generator_builder.prefixes.path', ''));
        $this->prefixes['view'] = explode('.', config('crud.generator_builder.prefixes.view', ''));
        $this->prefixes['public'] = explode('/', config('crud.generator_builder.prefixes.public', ''));
    ...
    {
        $this->addOns['swagger'] = config('crud.generator_builder.add_on.swagger', false);
        $this->addOns['tests'] = config('crud.generator_builder.add_on.tests', false);
        $this->addOns['datatables'] = config('crud.generator_builder.add_on.datatables', false);
        $this->addOns['menu.enabled'] = config('crud.generator_builder.add_on.menu.enabled', false);
        $this->addOns['menu.menu_file'] = config('crud.generator_builder.add_on.menu.menu_file',

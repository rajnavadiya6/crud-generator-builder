...
namespace Crud\GeneratorBuilder\Utils;
...
use Crud\GeneratorBuilder\Common\GeneratorFieldRelation;
...
class TableFieldsGenerator
{
    ...
    public function __construct($tableName, $ignoredFields, $connection = '')
    {
        ...
        $mappings = config('crud.generator_builder.from_table.doctrine_mappings', []);
        ...
        $this->defaultSearchable = config('crud.generator_builder.options.tables_searchable_default', false);
    }
    ...
    {
        if (! config('crud.generator_builder.timestamps.enabled', true)) {
            return [];
        }

        $createdAtName = config('crud.generator_builder.timestamps.created_at', 'created_at');
        $updatedAtName = config('crud.generator_builder.timestamps.updated_at', 'updated_at');
        $deletedAtName = config('crud.generator_builder.timestamps.deleted_at', 'deleted_at');
